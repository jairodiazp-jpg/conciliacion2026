...
class AdquirenciasConciliador:
    ...
    def _build_trace_marker(self, cruce_count: int, source: str) -> str:
        return f"TRACE-{source}-{cruce_count:03d}"
    ...
    def procesar(self) -> dict[str, Any]:
        ...
        for row in range(
            ...
            trace_marker = self._build_trace_marker(cruce_count, "ADQ")
            adquirencia_nombre = (
            ...
            observacion_adq = (
                f"{trace_marker} | {adquirencia_nombre} cruza con CCS Memorando (hoja {ccs_sheet.title.strip()}) | "
            ...
            for ccs_row in matches:
                ...
                observacion_ccs = (
                    f"{trace_marker} | {adquirencia_nombre} cruza con Adquirencias fila {row} | "
                    f"Autorización: {auth} | "
                    f"Valor: {value:.2f} | "
                    f"CCS fila: {ccs_row}"
                )
